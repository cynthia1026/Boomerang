...
export default function Result() {
  ...
  // ★ 현재 강조(하이라이트)된 시간 라벨들 (예: ['8.18 화 18:00', '8.18 화 18:30'])
  ...
  // 비고 카드 인터랙션 핸들러
  const handleNoteHoverStart = (note) => {
    const parsedLabels = typeof note.time_labels === 'string' ? JSON.parse(note.time_labels) : note.time_labels;
  ...
  const handleNoteTouch = (note) => {
    if (activeNoteId === note.id) {
      handleNoteHoverEnd();
    } else {
      handleNoteHoverStart(note);
    }
  };
      ...
      <section className="schedule-section">
        ...
        <Table 
          ...
          renderSlot={(dateIdx, timeIdx) => {
            ...
            // 현재 칸의 라벨 문자열 생성 (예: "8.18 화 18:00")
            ...
            return (
              <td 
                ...
                style={{ 
                  backgroundColor: isHighlighted ? '#444444' : getCellBgColor(count),
                }} 
              />
            );
          }} 
            ...
            <div className="result-notes-list">
              {roomNotes.map((n) => {
                ...
                return (
                  <div 
                    key={n.id} 
                    className={`result-note-item ${isActive ? 'active-hover' : ''}`}
                    onMouseEnter={() => handleNoteHoverStart(n)}
                    onMouseLeave={handleNoteHoverEnd}
                    onTouchStart={() => handleNoteTouch(n)}
                  >
